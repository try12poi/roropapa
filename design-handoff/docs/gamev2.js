...
const CFG={
  NIGHT_SEC:90,            // 하룻밤 길이(초): 03:00~06:00
  SPAWN_BASE:1.35,         // 좀비 생성 간격(초) 시작값 (클수록 쉬움)
  SPAWN_MIN:0.34,          // 최소 간격
  SPAWN_RAMP:0.010,        // 밤 지날수록 빨라지는 정도
  SPAWN_DAY:0.09,          // 날짜당 빨라지는 정도
  Z_HP:32, Z_HP_DAY:8, Z_HP_TIME:0.22,   // 좀비 체력
  Z_SPD:42, Z_SPD_DAY:2.0,               // 좀비 속도
  GNAW:2.4, GNAW_DAY:0.4,                 // 바리케이드 갉는 속도
...
{t:'사다리',x:936,y:154,w:48,h:46},
{t:'벙커입구',x:860,y:574,w:88,h:64}
];
...
function expResolve(n){
  ...
  let rescued=false;
  if(n.ring==='far'&&Math.random()<0.5){expLoot.rescue++;rescued=true;}
...
function reset(){ day=1; totalKills=0; food=20; materials=8; fac={barricade:0,farm:0}; ownedItems=[]; selMember=null; totalRescued=0; scene='1F'; startNight(true); }
function startNight(fresh){
  ...
  if(fresh){
    player={x:CENTER.x,y:CENTER.y+120,r:16,hp:CFG.P_HP,maxhp:CFG.P_HP,speed:CFG.P_SPD,
      ...
      swingT:0,swingDir:0,lungeT:0,painT:0,hurtCd:0,xp:0,lvl:1,need:6,repairing:null,pickR:60,away:false,rx:0,ry:0,ladCd:0};
    allies=[
...
cv.addEventListener('touchstart',e=>{const t=e.changedTouches[0];
  if(state==='assign'||state==='morning'||state==='expedition'){const rc=cv.getBoundingClientRect();const mx=(t.clientX-rc.left)*(VW/rc.width),my=(t.clientY-rc.top)*(VH/rc.height);if(state==='assign')assignTap(mx,my);else if(state==='morning')morningTap(mx,my);else expeditionTap(mx,my);e.preventDefault();return;}
  joy={id:t.identifier,sx:t.clientX,sy:t.clientY,dx:0,dy:0};e.preventDefault();},{passive:false});
cv.addEventListener('click',e=>{ if(state!=='assign'&&state!=='morning'&&state!=='expedition')return; const rc=cv.getBoundingClientRect();const mx=(e.clientX-rc.left)*(VW/rc.width),my=(e.clientY-rc.top)*(VH/rc.height);if(state==='assign')assignTap(mx,my);else if(state==='morning')morningTap(mx,my);else expeditionTap(mx,my); });
...
function endExpedition(){
  ...
  food+=expLoot.food; materials+=expLoot.mat; totalRescued+=expLoot.rescue;
...
function spawnZombie(){
  ...
  zombies.push({x,y,r:12,hp,maxhp:hp,speed:CFG.Z_SPD+Math.random()*12+day*CFG.Z_SPD_DAY,
...
function update(dt){
  ...
  const interval=Math.max(CFG.SPAWN_MIN, CFG.SPAWN_BASE-night_t*CFG.SPAWN_RAMP-day*CFG.SPAWN_DAY);
  ...
  if(player.cdLeft<=0){let hit=false,nd=1e9,ndir=0;
    for(const z of zombies){const d=Math.hypot(z.x-player.x,z.y-player.y);if(d<player.atkR+z.r&&losClear(player.x,player.y,z.x,z.y)){z.hp-=(player.eDmg||player.dmg);hit=true;const a=Math.atan2(z.y-player.y,z.x-player.x);z.x+=Math.cos(a)*10;z.y+=Math.sin(a)*10;if(d<nd){nd=d;ndir=a;}}}
  ...
  for(const a of allies){ if(a.down>0)continue;
    if(a.hp<a.maxhp && Math.hypot(a.x-player.x,a.y-player.y)<CFG.HEAL_R){
      a.hp=Math.min(a.maxhp,a.hp+(CFG.HEAL_RATE+(player.healBonus||0))*dt); player.healing=true;
  ...
  for(const a of allies){
    ...
    if(a.post==='roof'){ // 옥상 저격: 광역 사격, 근접 안전, 이동 없음
      ...
      if(tg&&a.cdLeft<=0){ a.cdLeft=(a.eCd||a.cd); tg.hp-=(a.eDmg||a.dmg); a.swingT=0.2; a.swingDir=Math.atan2(tg.y-a.y,tg.x-a.x);
        fx.push({type:'shot',x1:a.x,y1:a.y-10,x2:tg.x,y2:tg.y,t:0.12}); }
      continue;
    }
    a.onRoof=false;
    ...
    if(a.key==='mansu'){ // 설비기사: 근처 좀비는 렌치로 요격, 없으면 배치 거점 수리
      ...
      if(tg){ td=Math.hypot(tg.x-a.x,tg.y-a.y);
        ...
        if(w&&w.barr.hp<w.barr.max){ if(goHold(a,w,w.inP.x,w.inP.y,26)<=26) w.barr.hp=Math.min(w.barr.max,w.barr.hp+(CFG.MANSU_REPAIR+(a.eRepair||0))*dt); }
        else if(hold){ goHold(a,post,hold.x,hold.y,20); }
      }
      collideW(a); continue;
    }
    // 전투: 거점 배치면 거점 반경 안 좀비 능동 요격. 거점에 가장 가까운(=제일 위협적인) 좀비 우선.
    const RNG=a.eRng||a.rng, DMG=a.eDmg||a.dmg, CDv=a.eCd||a.cd;
    ...
    if(tg){ td=Math.hypot(tg.x-a.x,tg.y-a.y);   // 실제 사거리 판정은 나-좀비 거리로
      ...
      if(needMove&&tooFar){ if(hold)goHold(a,post,hold.x,hold.y,20); }   // 너무 멀어졌으면 복귀 우선
        ...
        if(a.eRanged)fx.push({type:'shot',x1:a.x,y1:a.y-10,x2:tg.x,y2:tg.y,t:0.1});}
  ...
  for(let i=zombies.length-1;i>=0;i--){
    ...
    if(z.inside){
      ...
      let en=null,bc=1e9;
      for(const e of ENTRIES){const c=Math.hypot(z.x-e.cx,z.y-e.cy)+Math.hypot(e.inP.x-T.x,e.inP.y-T.y)+(e.barr.hp>0?260:0);if(c<bc){bc=c;en=e;}}
      z.entry=en;
      ...
      if(en.barr.hp>0){
        ...
        else if(en.isWin && z.climb<CFG.WINDOW_CLIMB){ z.climb+=dt; }
    ...
    // 동료 타격
    for(const a of allies){if(a.down>0||a.post==='roof'||(a.work&&a.work!=='combat'))continue;const ad=Math.hypot(z.x-a.x,z.y-a.y);if(ad<z.r+a.r&&z.hitCd<=0){z.hitCd=0.8;a.hp-=(11+day*1.2);if(a.hp<=0){a.hp=0;a.down=10;}}}
  ...
  if(player.hp<=0){player.hp=0;return gameOver('서연이 쓰러졌다');}
  if(night_t>=CFG.NIGHT_SEC)return morning();
}
function morning(){state='dawn';dawnT=0;}
let morningInfo={};
function realMorning(){
  ...
  if(morningInfo.starve){ for(const a of allies)a.hp=Math.max(1,Math.round(a.hp*0.7)); } // 굶주림 패널티
  recomputeCombos();
  state='morning';
}
function gameOver(t){state='over';document.getElementById('oDday').textContent='D+'+day;document.getElementById('oTitle').textContent=t;document.getElementById('oStats').textContent=day+'일 밤까지 생존 · 처치 '+totalKills+' · Lv.'+player.lvl;document.getElementById('over').classList.remove('hidden');}
...
function render(){
  ...
  // 밤 어둠
  const prog=night_t/CFG.NIGHT_SEC; cx.fillStyle='rgba(6,10,26,'+(0.40*Math.sin(Math.min(1,prog)*Math.PI))+')'; cx.fillRect(camX,camY,vw,vh);
  // 바리케이드 그리기 (출입구)
  ...
  // 좀비
  for(const z of zombies){const bob=Math.sin(z.bob)*2;const climbing=(!z.inside&&z.entry.isWin&&z.entry.barr.hp<=0&&z.climb>0&&z.climb<CFG.WINDOW_CLIMB);
    ...
    cx.fillStyle='#FF5A5A';cx.beginPath();cx.arc(z.x-4,z.y+bob-2,2.2,0,6.29);cx.fill();cx.beginPath();cx.arc(z.x+4,z.y+bob-2,2,0,6.29);cx.fill();cx.restore();
...
function loop(ts){if(lastTs===undefined)lastTs=ts;let dt=(ts-lastTs)/1000;lastTs=ts;if(dt>0.05)dt=0.05;
  ...
  if(state==='play'||state==='levelup'||state==='pause'||state==='dawn'||state==='assign'||state==='morning'||state==='expedition')render();
