...
const CFG={
  NIGHT_SEC:420,           // 하룻밤 420초 = 03:00~06:00
  SPAWN_BASE:1.7,          // v4: 난이도 상향 (2.8→1.7, 물량 65%↑)
  SPAWN_MIN:0.32,
  SPAWN_RAMP:0.006,
  SPAWN_DAY:0.12,
  Z_HP:48, Z_HP_DAY:12, Z_HP_TIME:0.06,
  Z_SPD:28, Z_SPD_DAY:1.4,
  GNAW:3.4, GNAW_DAY:0.5,                 // 바리케이드 갉는 속도
...
const OBJ=[
{t:'벽',x:228,y:284,w:472,h:56},
{t:'문',x:690,y:338,w:24,h:87},
{t:'사물',x:352,y:598,w:57,h:330},
{t:'사물',x:462,y:598,w:66,h:330},
{t:'사물',x:568,y:598,w:66,h:330},
{t:'사물',x:435,y:969,w:162,h:54},
{t:'벽',x:690,y:286,w:22,h:50},
{t:'벙커입구',x:821,y:596,w:149,h:107},
{t:'벽',x:664,y:31,w:22,h:257},
{t:'벽',x:687,y:26,w:18,h:139},
{t:'벽',x:913,y:29,w:26,h:135},
{t:'벽',x:937,y:29,w:79,h:15},
{t:'문',x:708,y:33,w:202,h:129},
{t:'벽',x:998,y:30,w:21,h:704},
{t:'벽',x:192,y:1054,w:28,h:157},
{t:'벽',x:200,y:884,w:26,h:52},
{t:'벽',x:207,y:682,w:31,h:200},
{t:'벽',x:220,y:309,w:22,h:284},
{t:'벽',x:315,y:424,w:434,h:110},
{t:'벽',x:728,y:734,w:291,h:94},
{t:'벽',x:722,y:539,w:31,h:191},
{t:'벽',x:887,y:1090,w:132,h:120},
{t:'벽',x:984,y:830,w:38,h:256},
{t:'벽',x:415,y:1083,w:33,h:131},
{t:'벽',x:643,y:1084,w:30,h:132},
{t:'창문',x:677,y:1097,w:210,h:111},
{t:'창문',x:221,y:1096,w:194,h:112},
{t:'창문',x:198,y:945,w:28,h:106},
{t:'창문',x:209,y:598,w:36,h:81},
{t:'문',x:244,y:430,w:72,h:108},
{t:'문',x:450,y:1085,w:190,h:125},
{t:'사물',x:853,y:259,w:117,h:290},
{t:'사물',x:736,y:831,w:61,h:159},
{t:'사물',x:231,y:742,w:53,h:188},
{t:'사물',x:39,y:401,w:82,h:177},
{t:'사물',x:31,y:617,w:73,h:187},
{t:'사물',x:15,y:853,w:81,h:191},
{t:'사물',x:874,y:1290,w:101,h:59},
{t:'사물',x:665,y:1291,w:35,h:61},
{t:'사다리',x:720,y:210,w:40,h:70}
];
...
const CENTER={x:890*S, y:930*S};   // v4: 사장님 벽 데이터 기준 최대 여유 지점
...
let ENTRIES=OPEN.map((o,i)=>{
  ...
  // v4: 안쪽 지점이 벽에 끼면 거리를 늘려가며 뚫린 자리 탐색
  let inP={x:cxm+Math.cos(ang)*46, y:cym+Math.sin(ang)*46};
  {
    const tryP=(dist,a)=>({x:cxm+Math.cos(a)*dist, y:cym+Math.sin(a)*dist});
    const isFree=(pt)=>{ const q={x:pt.x,y:pt.y,r:20}; collide(q);
      return Math.hypot(q.x-pt.x,q.y-pt.y)<2; };
    if(!isFree(inP)){
      let found=null;
      for(const dist of [60,76,92,110,130]){
        for(const off of [0,0.3,-0.3,0.6,-0.6,0.9,-0.9]){
          const cand=tryP(dist,ang+off);
          if(isFree(cand)){ found=cand; break; }
        }
        if(found)break;
      }
      if(found)inP=found;
...
const INNER_DOORS=[];
{
  ...
  for(const e of ENTRIES){
    // v4: 큰 문(정문·셔터)은 무조건 거점 — 외부 연결 판정 실패해도 채택
    const big = (e.rect.w*e.rect.h > 14000);
    e.perimeter = big || nearOut(e.outP.x,e.outP.y) || nearOut(e.cx,e.cy);
    if(!e.perimeter)INNER_DOORS.push(e);
  }
}
...
// ===== v4: 호드 =====
function isHorde(d){ return d%7===0; }
// ===== v4: E10 탈출 =====
let deepOpen=false, escape=null, endAnimT=0, endPage=0;
function aliveKey(k){ return allies.some(a=>a.key===k) || k==='seoyeon'; }
function escapeReady(){ return escape&&escape.truck&&escape.fuel&&escape.route; }
function hordeIn(){ const n=7-(day%7); return n===7?0:n; }   // 0 = 오늘이 호드
// ===== v4: 실내 전투(하이브리드 원정) =====
...
function reset(){ day=1; totalKills=0; food=20; materials=8; fac={barricade:0,farm:0}; ownedItems=[]; selMember=null; totalRescued=0; joined=[]; memorial=[]; rescueCard=null; pendingRescue=[]; escortees=[]; memorialView=null; morale=70; blackout=false; prevBlackout=false; moraleFx=[]; indoor=null; journalView=null; sp=0; skills={}; journals=0; keyLevel=1; skillSel='seoyeon'; deepOpen=false; escape=null; endAnimT=0; endPage=0; scene='1F'; startNight(true); }
function startNight(fresh){
  night_t=0; kills=0; zombies=[]; gems=[]; fx=[]; spawnAcc=0;
  ripples=[]; knock=null; knockDone=false; variantAcc=0; hordeQueue=[];
  ...
  if(fresh){
    player={x:CENTER.x,y:CENTER.y+120,r:20,hp:CFG.P_HP,maxhp:CFG.P_HP,speed:CFG.P_SPD,
    ...
    ].map(a=>({...a,r:20,x:CENTER.x+(Math.random()*80-40),y:CENTER.y+(Math.random()*60),maxhp:a.hp,baseMaxhp:a.hp,baseMaxhp0:a.hp,
  ...
  if(escape&&escape.night) fx.push({type:'alert',txt:'실행의 밤 — 셔터를 연다',t:3.0});
  else if(isHorde(day)) fx.push({type:'alert',txt:'호드 — 물량 2.5배',t:2.6,bad:true});
  if(isHorde(day)&&!blackout){   // v4: 호드 — 이삿짐 1~3 고정
    const n=1+Math.floor(day/14);
    for(let i=0;i<Math.min(3,n);i++) hordeQueue.push(30+i*40);
  }
...
cv.addEventListener('touchstart',e=>{const t=e.changedTouches[0];
  ...
  if(state==='assign'||state==='morning'||state==='expedition'||state==='deepest'||state==='ending'){const rc=cv.getBoundingClientRect();const mx=(t.clientX-rc.left)*(VW/rc.width),my=(t.clientY-rc.top)*(VH/rc.height);if(state==='deepest')deepestTap(mx,my);else if(state==='ending')endingTap();else if(state==='assign')assignTap(mx,my);else if(state==='morning')morningTap(mx,my);else if(state==='memorial')memorialTap(mx,my);else if(state==='skill')skillTap(mx,my);else if(state==='journal')journalTap(mx,my);else expeditionTap(mx,my);e.preventDefault();return;}
...
cv.addEventListener('click',e=>{
  ...
  if(state!=='assign'&&state!=='morning'&&state!=='expedition'&&state!=='memorial'&&state!=='skill'&&state!=='journal'&&state!=='deepest'&&state!=='ending')return; const rc=cv.getBoundingClientRect();const mx=(e.clientX-rc.left)*(VW/rc.width),my=(e.clientY-rc.top)*(VH/rc.height);if(state==='deepest')deepestTap(mx,my);else if(state==='ending')endingTap();else if(state==='assign')assignTap(mx,my);else if(state==='morning')morningTap(mx,my);else if(state==='memorial')memorialTap(mx,my);else if(state==='skill')skillTap(mx,my);else if(state==='journal')journalTap(mx,my);else expeditionTap(mx,my); });
...
function drawAssign(){
  ...
  cx.fillStyle='#08131a';cx.font='900 17px "Apple SD Gothic Neo",sans-serif';cx.textAlign='center';cx.font='900 20px "Apple SD Gothic Neo",sans-serif';cx.fillText('완료 · 방어 시작',VW/2,L.done.y+L.done.h/2+7);
}
...
function morningLayout(){
  ...
  const lightY=itemY+60;
  const eq=escape?[{k:'truck',x,y:lightY+66,w:w/3-6,h:46},{k:'fuel',x:x+w/3+3,y:lightY+66,w:w/3-6,h:46},{k:'route',x:x+2*w/3+6,y:lightY+66,w:w/3-6,h:46}]:[];
  return {rows,fac2,chips,eq,next:{x,y:VH-72,w,h:60},itemY,skillBtn:{x:VW-124,y:70,w:112,h:38},
    light:{on:{x,y:lightY,w:w/2-4,h:58}, off:{x:x+w/2+4,y:lightY,w:w/2-4,h:58}}};
}
function drawMorning(){
  ...
  { const L2=morningLayout(), b=L2.skillBtn;
    ...
    cx.font='800 15px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('✨ 스킬'+(sp>0?' ('+sp+')':''),b.x+b.w/2,b.y+25);
  ...
  // v4: 호드 예고
  { const n=hordeIn();
    cx.textAlign='left'; cx.font='800 11px "Apple SD Gothic Neo",sans-serif';
    if(n===0){ cx.fillStyle='#FF5A5A'; cx.fillText('⚠ 오늘 밤 호드 — 물량 2.5배 · 이삿짐 등장',12,82); }
    else if(n===1){ cx.fillStyle='#FFC24B'; cx.fillText('⚠ 내일 밤 호드 — 오늘 바리케이드를 올려라',12,82); }
    else { cx.fillStyle='#8CA0B3'; cx.fillText('호드까지 '+n+'일 · 생존자 탭=아이템 · 근무(전투/시설/휴식)',12,82); } }
  const L=morningLayout();
  // v4: 탈출 준비 패널
  if(escape){ const rdy=escapeReady();
    cx.textAlign='left'; cx.fillStyle=rdy?'#FFC24B':'#C9B27A'; cx.font='800 11px "Apple SD Gothic Neo",sans-serif';
    cx.fillText(rdy?'🚚 탈출 준비 완료 — 오늘 밤 떠난다':'🚚 탈출 준비',12,L.eq[0].y-6);
    for(const e of L.eq){ const q=ESCQ[e.k], done=escape[e.k];
      const can=!done&&e.k!=='fuel'&&(!q.who||q.who.some(aliveKey))&&materials>=q.cost;
      cx.fillStyle=done?'rgba(126,216,168,0.18)':can?'rgba(255,194,75,0.14)':'rgba(18,26,43,0.85)';
      cx.strokeStyle=done?'#7ED8A8':can?'#FFC24B':'#243247'; cx.lineWidth=done||can?2:1;
      cx.fillRect(e.x,e.y,e.w,e.h); cx.strokeRect(e.x,e.y,e.w,e.h);
      cx.textAlign='center'; cx.fillStyle=done?'#7ED8A8':can?'#FFC24B':'#5a6b80';
      cx.font='800 13px "Apple SD Gothic Neo",sans-serif';
      cx.fillText((done?'✔ ':'')+q.nm,e.x+e.w/2,e.y+20);
      cx.font='9px "Apple SD Gothic Neo",sans-serif'; cx.fillStyle=done?'#5a7a68':'#5a6b80';
      cx.fillText(done?'완료':q.hint,e.x+e.w/2,e.y+36);
    }
  }
  const WKN=['전투','시설','휴식'];
  ...
  { const LB=L.light;
    ...
    cx.font='800 15px "Apple SD Gothic Neo",sans-serif';cx.fillText('💡 점등 — 등대가 된다',LB.on.x+LB.on.w/2,LB.on.y+24);
    cx.fillStyle=onSel?'#C9B27A':'#5a6b80';cx.font='9px "Apple SD Gothic Neo",sans-serif';
    cx.font='10px "Apple SD Gothic Neo",sans-serif';cx.fillText('웨이브 100% · 생존자 유입',LB.on.x+LB.on.w/2,LB.on.y+43);
    ...
    cx.font='800 15px "Apple SD Gothic Neo",sans-serif';cx.fillText('🌑 소등의 밤 — 숨는다',LB.off.x+LB.off.w/2,LB.off.y+24);
    cx.fillStyle=offSel?'#7FB4FF':'#5a6b80';cx.font='9px "Apple SD Gothic Neo",sans-serif';
    cx.font='10px "Apple SD Gothic Neo",sans-serif';cx.fillText('웨이브 −70% · 사기'+(prevBlackout?'−20':'−10'),LB.off.x+LB.off.w/2,LB.off.y+43);
  }
  // 다음 밤
  const goEsc=escapeReady();
  cx.fillStyle=goEsc?'#7ED8A8':'#FFC24B';cx.fillRect(L.next.x,L.next.y,L.next.w,L.next.h);
  cx.fillStyle=goEsc?'#06231a':'#1a1206';cx.font='900 16px "Apple SD Gothic Neo",sans-serif';cx.textAlign='center';cx.font='900 20px "Apple SD Gothic Neo",sans-serif';
  cx.fillText(goEsc?'🚚 오늘 밤 떠난다 — 실행의 밤':'☀️ 낮 원정 출발',VW/2,L.next.y+L.next.h/2+7);
}
// ===== v4: 점장의 일지 화면 =====
...
function journalTap(lx,ly){
  const B=jrnBtn();
  if(lx>B.x&&lx<B.x+B.w&&ly>B.y&&ly<B.y+B.h){ journalView=null; state=(journals>=12&&keyLevel>=4&&!deepOpen)?(deepOpen=true,'deepest'):'morning'; }
}
// ===== v4: 스킬 트리 화면 =====
function skillLayout(){
  ...
  ['A','B'].forEach((br,bi)=>{
    TREES[skillSel][br].nodes.forEach((nd,ni)=>{
      cells.push({br,ni,nd,x:pad+bi*(colW+14),y:top+34+ni*96,w:colW,h:86});
    });
  });
  return {tabs,cells,colW,top,pad,back:{x:pad,y:VH-70,w:w,h:58}};
}
function drawSkill(){
  cx.fillStyle='rgba(6,10,18,0.98)'; cx.fillRect(0,0,VW,VH);
  ...
  cx.font='900 18px "Apple SD Gothic Neo",sans-serif';cx.fillText('아침으로 돌아가기',VW/2,L.back.y+36);
}
...
function drawExpedition(){
  ...
  if(escortees.length){ cx.textAlign='left'; cx.fillStyle='#7FE3F0'; cx.font='700 12px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('🤝 호위 중: '+escortees.map(e=>e.nm).join(', '),12,VH-124); }
  { const HB=homeBtn();
    cx.fillStyle='#7FE3F0'; roundRect(HB.x,HB.y,HB.w,HB.h,12); cx.fill();
    cx.fillStyle='#08131a'; cx.font='900 19px "Apple SD Gothic Neo",sans-serif'; cx.textAlign='center';
    cx.fillText('🏪 편의점으로 귀환',HB.x+HB.w/2,HB.y+36); }
...
function endExpedition(){
  ...
  for(const e of escortees){
    ...
    allies.push({key:e.key,nm:e.nm,ranged:!!e.ranged,rng:e.rng,dmg:e.dmg,cd:e.cd,hp:e.hp,spd:e.spd,
      r:20,x:CENTER.x+(Math.random()*80-40),y:CENTER.y+(Math.random()*60),maxhp:e.hp,baseMaxhp:e.hp,baseMaxhp0:e.hp,
  ...
  if(escape&&!escape.fuel){ escape.fuel=true; fx.push({type:'alert',txt:'연료 확보 — 탈출 준비 1/3',t:2.4}); }
  escortees=[]; rescueCard=null;
  startNight(false); state='play';
}
// ===== v4: 구출 정보 카드 =====
function cardLayout(){ const w=Math.min(340,VW-30), x=(VW-w)/2, h=310, y=(VH-h)/2;
  return {x,y,w,h, take:{x:x+14,y:y+h-70,w:w/2-20,h:56}, pass:{x:x+w/2+6,y:y+h-70,w:w/2-20,h:56}}; }
function drawRescueCard(){
  ...
  cx.font='900 17px "Apple SD Gothic Neo",sans-serif';cx.fillText('데려간다',L.take.x+L.take.w/2,L.take.y+35);
  ...
  cx.fillStyle='#9fb0c4'; cx.fillText('지나친다',L.pass.x+L.pass.w/2,L.pass.y+35);
}
...
function expeditionTap(lx,ly){
  ...
  const HB=homeBtn();
  if(lx>HB.x&&lx<HB.x+HB.w&&ly>HB.y&&ly<HB.y+HB.h){ endExpedition(); return; }   // v4: 큰 귀환 버튼
  if(Math.hypot(lx-EXP_BASE.x,ly-EXP_BASE.y)<52){ endExpedition(); return; }     // 판정 24→52
...
function morningTap(lx,ly){
  const L=morningLayout();
  const N=L.next; if(lx>N.x&&lx<N.x+N.w&&ly>N.y&&ly<N.y+N.h){
    if(escapeReady()){ escape.night=true; document.getElementById('morning').classList.add('hidden'); startNight(false); state='play'; return; }
    genExpedition(); state='expedition'; return; }
  ...
  for(const e of L.eq){ if(lx>e.x&&lx<e.x+e.w&&ly>e.y&&ly<e.y+e.h){ escTap(e.k); return; } }
}
// v4: 탈출 준비 3과제
const ESCQ={
  truck:{nm:'트럭 정비',cost:15,who:['mansu','sangcheol'],hint:'자재 15 · 만수/상철 필요'},
  fuel :{nm:'연료 확보',cost:0, who:null,            hint:'원정 1회 완수로 획득'},
  route:{nm:'루트 확보',cost:5, who:['mika','sangcheol'],hint:'자재 5 · 미카/상철 필요'}
};
function escTap(k){
  if(!escape||escape[k])return;
  const q=ESCQ[k];
  if(k==='fuel')return;                                  // 원정에서만
  if(q.who&&!q.who.some(aliveKey))return;                // 담당자 생존 필요
  if(materials<q.cost)return;
  materials-=q.cost; escape[k]=true; addMorale(5,'탈출 준비');
  moraleFx.push({txt:q.nm+' 완료',col:'#FFC24B',t:2.2});
}

let spawnRR=0, variantAcc=0, hordeQueue=[];
function spawnZombie(){
  // v4 수정: 거점을 라운드로빈으로 균등 배정 → 좌상단 편중 해소
  const best=ENTRIES[spawnRR++%ENTRIES.length];
  // 그 거점에 가까운 맵 가장자리에서 스폰
  let x,y;
  const cxr=best.cx, cyr=best.cy;
  const dTop=cyr, dBot=WH-cyr, dLeft=cxr, dRight=WW-cxr;
  const m=Math.min(dTop,dBot,dLeft,dRight);
  const jit=()=>(Math.random()*300-150);
  if(m===dTop){x=Math.max(20,Math.min(WW-20,cxr+jit()));y=8;}
  else if(m===dBot){x=Math.max(20,Math.min(WW-20,cxr+jit()));y=WH-8;}
  else if(m===dLeft){x=8;y=Math.max(20,Math.min(WH-20,cyr+jit()));}
...
// v4: 이삿짐(브루트) — HP 4배, 느림, 갉기 5배, 넉백 면역
function spawnBrute(){
  const en=ENTRIES[Math.floor(Math.random()*ENTRIES.length)];
  const m=Math.min(WW-20,Math.max(20,en.cx+(Math.random()*200-100)));
  const y=(en.cy<WH/2)?8:WH-8;
  const hp=(CFG.Z_HP+day*CFG.Z_HP_DAY+night_t*CFG.Z_HP_TIME)*4;
  zombies.push({x:m,y,r:22,hp,maxhp:hp,speed:24,kind:'brute',gnawMul:5,noKb:true,
    entry:en,inside:false,climb:0,hitCd:0,bob:Math.random()*6,stun:0,slow:0});
}
// v4: 배달통(부머) — 사망 시 국물 폭발(주변 좀비 가속)
function spawnBoomer(){
  const en=ENTRIES[Math.floor(Math.random()*ENTRIES.length)];
  const m=Math.min(WW-20,Math.max(20,en.cx+(Math.random()*200-100)));
  const y=(en.cy<WH/2)?8:WH-8;
  const hp=(CFG.Z_HP+day*CFG.Z_HP_DAY+night_t*CFG.Z_HP_TIME)*0.8;
  zombies.push({x:m,y,r:18,hp,maxhp:hp,speed:CFG.Z_SPD+day*CFG.Z_SPD_DAY,kind:'boomer',
    entry:en,inside:false,climb:0,hitCd:0,bob:Math.random()*6,stun:0,slow:0});
}
...
function update(dt){
  ...
  // ===== v4: 변종 주기 스폰 =====
  for(let i=hordeQueue.length-1;i>=0;i--){ if(night_t>=hordeQueue[i]){ hordeQueue.splice(i,1); spawnBrute(); } }
  variantAcc+=dt;
  if(variantAcc>6){ variantAcc=0;
    if(day>=4&&Math.random()<0.35) spawnBoomer();
    if(day>=5&&Math.random()<0.25) spawnBrute();
  }
  ...
  if(isHorde(day))interval/=2.5;                                                // v4: 호드 밤 — 물량 2.5배
      ...
      if(sprayCd<=0){ let n=0;
        for(const z of zombies){ if(n>=3)continue;
          ...
          if(d<130){ const ang=Math.atan2(z.y-jh.y,z.x-jh.x);
            if(!z.noKb){ z.x+=Math.cos(ang)*70; z.y+=Math.sin(ang)*70; } z.slow=3; n++;
        ...
        if(tg){ const ang=Math.atan2(tg.y-sc.y,tg.x-sc.x); let n=0;
          for(const z of zombies){ const d=Math.hypot(z.x-sc.x,z.y-sc.y);
            ...
            if(Math.abs(((a2-ang+Math.PI*3)%(Math.PI*2))-Math.PI)<0.35){
              if(!z.noKb){ z.x+=Math.cos(ang)*90; z.y+=Math.sin(ang)*90; } z.hp-=18; z.slow=2; n++; } }
  ...
  for(let i=zombies.length-1;i>=0;i--){
    ...
    const _sp=z.speed; if(z.slow>0){ z.slow-=dt; z.speed*=0.7; } if(z.boost>0){ z.boost-=dt; z.speed*=1.3; } // v4: 둔화·가속
    if(z.inside){
      ...
      if(en.barr.hp>0){
        ...
        else {const was=en.barr.hp;en.barr.hp-=(CFG.GNAW+day*CFG.GNAW_DAY)*(z.gnawMul||1)*dt;   // 문 앞 도착 → 부수는 중
    ...
    if(z.hp<=0){kills++;totalKills++;fx.push({type:'pop',x:z.x,y:z.y,t:0.3});
      if(z.kind==='boomer'){   // v4: 국물 폭발 — 주변 좀비 8초 가속
        for(const z2 of zombies){ if(z2!==z&&Math.hypot(z2.x-z.x,z2.y-z.y)<60){ z2.boost=8; } }
        fx.push({type:'float',x:z.x,y:z.y-24,txt:'국물 폭발!',col:'#FFC24B',t:1.2}); shake=4; }
...
function morning(){
  if(escape&&escape.night){ endPage=0; endAnimT=0; state='ending'; return; }   // v4: E10 탈출 엔딩
...
function drawMemorial(){
  ...
  cx.fillStyle='#7FE3F0'; roundRect(VW/2-105,VH-80,210,56,12); cx.fill();
...
// ===== v4: E10 최심부 개방 화면 =====
function deepestLayout(){ return {stay:{x:24,y:VH-86,w:(VW-60)/2,h:62}, go:{x:VW-24-(VW-60)/2,y:VH-86,w:(VW-60)/2,h:62}}; }
function drawDeepest(){
  cx.fillStyle='#070B12'; cx.fillRect(0,0,VW,VH);
  // 전국 지점 지도 — 램프 점멸
  cx.strokeStyle='rgba(127,227,240,0.18)'; cx.lineWidth=1;
  cx.strokeRect(40,96,VW-80,VH-250);
  const lamps=[[0.32,0.18],[0.55,0.31],[0.44,0.52],[0.68,0.63],[0.28,0.74],[0.75,0.42],[0.50,0.86]];
  for(let i=0;i<lamps.length;i++){
    const lx2=40+lamps[i][0]*(VW-80), ly2=96+lamps[i][1]*(VH-250);
    const pulse=0.45+0.55*Math.abs(Math.sin(endAnimT*1.6+i));
    cx.fillStyle='rgba(255,194,75,'+pulse.toFixed(2)+')';
    cx.beginPath(); cx.arc(lx2,ly2,4,0,6.29); cx.fill();
    cx.strokeStyle='rgba(255,194,75,'+(pulse*0.3).toFixed(2)+')'; cx.lineWidth=1;
    cx.beginPath(); cx.arc(lx2,ly2,10+pulse*7,0,6.29); cx.stroke();
  }
  cx.textAlign='center';
  cx.fillStyle='#FFC24B'; cx.font='900 21px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('벙커 최심부 — 개방',VW/2,56);
  cx.fillStyle='#8CA0B3'; cx.font='12px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('무기도 금도 아니었다. 전국 안심24 지점 지도 · 본사 비상 프로토콜 · 일지 12장(음성)',VW/2,78);
  cx.fillStyle='#7FE3F0'; cx.font='800 16px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('서연: "…등대가, 우리만이 아니었어."',VW/2,VH-160);
  cx.fillStyle='#C9B27A'; cx.font='12px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('"지켜라. 사람을 받아라. 밥을 나눠라. 그러다 때가 되면 — 떠나라."  — 강대식',VW/2,VH-134);
  const L=deepestLayout();
  cx.fillStyle='rgba(30,41,61,0.95)'; cx.strokeStyle='#3a4658'; cx.lineWidth=2;
  cx.fillRect(L.stay.x,L.stay.y,L.stay.w,L.stay.h); cx.strokeRect(L.stay.x,L.stay.y,L.stay.w,L.stay.h);
  cx.fillStyle='#E8EFF7'; cx.font='900 17px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('남는다',L.stay.x+L.stay.w/2,L.stay.y+30);
  cx.fillStyle='#8CA0B3'; cx.font='10px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('무한 생존 계속 · 지도는 벙커에 걸린다',L.stay.x+L.stay.w/2,L.stay.y+48);
  cx.fillStyle='rgba(255,194,75,0.2)'; cx.strokeStyle='#FFC24B';
  cx.fillRect(L.go.x,L.go.y,L.go.w,L.go.h); cx.strokeRect(L.go.x,L.go.y,L.go.w,L.go.h);
  cx.fillStyle='#FFC24B'; cx.font='900 17px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('떠난다',L.go.x+L.go.w/2,L.go.y+30);
  cx.fillStyle='#C9B27A'; cx.font='10px "Apple SD Gothic Neo",sans-serif';
  cx.fillText('탈출 준비 개시 — 트럭·연료·루트',L.go.x+L.go.w/2,L.go.y+48);
}
function deepestTap(lx,ly){
  const L=deepestLayout();
  if(lx>L.stay.x&&lx<L.stay.x+L.stay.w&&ly>L.stay.y&&ly<L.stay.y+L.stay.h){ state='morning'; return; }
  if(lx>L.go.x&&lx<L.go.x+L.go.w&&ly>L.go.y&&ly<L.go.y+L.go.h){
    escape={truck:false,fuel:false,route:false,night:false}; addMorale(10,'희망'); state='morning'; return; }
}
// ===== v4: E10 롤콜 엔딩 =====
function drawEnding(){
  cx.fillStyle='#05080E'; cx.fillRect(0,0,VW,VH);
  // 새벽 도로 — 흘러가는 차선
  cx.strokeStyle='rgba(127,180,255,0.18)'; cx.lineWidth=3;
  for(let i=0;i<7;i++){ const yy=((endAnimT*160+i*90)%(VH+90))-45;
    cx.beginPath(); cx.moveTo(VW/2-3,yy); cx.lineTo(VW/2-3,yy+42); cx.stroke(); }
  cx.textAlign='center';
  if(endPage===0){   // 추모 폴라로이드 롤콜
    cx.fillStyle='#C9B27A'; cx.font='700 12px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('두고 온 얼굴들',VW/2,54);
    const list=memorial;
    if(!list.length){ cx.fillStyle='#7ED8A8'; cx.font='900 18px "Apple SD Gothic Neo",sans-serif';
      cx.fillText('아무도 두고 오지 않았다.',VW/2,VH/2); }
    for(let i=0;i<list.length;i++){
      const ph=(endAnimT-i*1.1);
      if(ph<0||ph>2.2) continue;
      const a=ph<0.3?ph/0.3:(ph>1.8?(2.2-ph)/0.4:1);
      const py=VH*0.62-ph*40;
      cx.save(); cx.globalAlpha=Math.max(0,Math.min(1,a));
      cx.translate(VW/2,py); cx.rotate((i%2?1:-1)*0.05);
      cx.fillStyle='#F5EFE0'; cx.fillRect(-52,-64,104,128);
      cx.fillStyle='#2A3346'; cx.fillRect(-44,-56,88,84);
      cx.fillStyle='#3a4658'; cx.font='900 15px "Apple SD Gothic Neo",sans-serif';
      cx.fillText(list[i].nm,0,48);
      cx.fillStyle='#7a6a52'; cx.font='9px "Apple SD Gothic Neo",sans-serif';
      cx.fillText('D+'+list[i].from+'~D+'+list[i].to+' · '+(list[i].to-list[i].from+1)+'일',0,60);
      cx.restore();
      cx.fillStyle='rgba(232,239,247,'+Math.max(0,Math.min(1,a)).toFixed(2)+')';
      cx.font='12px "Apple SD Gothic Neo",sans-serif';
      cx.fillText('"'+list[i].memo+'"',VW/2,py+92);
    }
  } else if(endPage===1){   // 탑승자 명단
    cx.fillStyle='#7ED8A8'; cx.font='900 17px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('탑승자 명단',VW/2,64);
    const crew=['서연'].concat(allies.map(a=>a.nm));
    for(let i=0;i<crew.length;i++){
      const a=Math.max(0,Math.min(1,(endAnimT-i*0.22)/0.4));
      cx.fillStyle='rgba(232,239,247,'+a.toFixed(2)+')';
      cx.font='800 15px "Apple SD Gothic Neo",sans-serif';
      cx.fillText(crew[i],VW/2,100+i*26);
    }
  } else if(endPage===2){   // 정산
    cx.fillStyle='#E8EFF7'; cx.font='900 22px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('D+'+day+'.',VW/2,VH/2-60);
    cx.font='900 19px "Apple SD Gothic Neo",sans-serif';
    cx.fillStyle='#7ED8A8'; cx.fillText('함께 떠난 사람 '+(allies.length+1)+'명.',VW/2,VH/2-16);
    cx.fillStyle='#C9B27A'; cx.fillText('두고 온 얼굴 '+memorial.length+'장.',VW/2,VH/2+18);
    cx.fillStyle='#8CA0B3'; cx.font='12px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('처치 '+totalKills+' · 구출 '+totalRescued+' · 최종 사기 '+Math.round(morale),VW/2,VH/2+56);
  } else {   // 백미러 — 불은 켜둔 채
    const gl=0.5+0.5*Math.abs(Math.sin(endAnimT*1.1));
    cx.save(); cx.translate(VW/2,VH*0.42);
    cx.fillStyle='rgba(255,194,75,'+(gl*0.14).toFixed(2)+')';
    cx.beginPath(); cx.arc(0,0,74,0,6.29); cx.fill();
    cx.fillStyle='#6E2B8E'; cx.fillRect(-40,-16,80,26);
    cx.fillStyle='#9BDE3C'; cx.fillRect(-40,10,80,5);
    cx.fillStyle='rgba(255,240,200,'+gl.toFixed(2)+')';
    cx.font='900 13px "Apple SD Gothic Neo",sans-serif'; cx.fillText('안심24',0,3);
    cx.restore();
    cx.fillStyle='#5a6b80'; cx.font='11px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('백미러 속 안심24. 불은 켜둔 채.',VW/2,VH*0.42+108);
    const a=Math.max(0,Math.min(1,(endAnimT-1.2)/1.4));
    cx.fillStyle='rgba(255,194,75,'+a.toFixed(2)+')';
    cx.font='900 17px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('"불빛은 사람을 부르고,',VW/2,VH-146);
    cx.fillText('사람이 모이면 아침이 온다."',VW/2,VH-120);
    const a2=Math.max(0,Math.min(1,(endAnimT-3.0)/1.2));
    cx.fillStyle='rgba(127,227,240,'+a2.toFixed(2)+')';
    cx.font='800 13px "Apple SD Gothic Neo",sans-serif';
    cx.fillText('다음 등대: 인천공항점',VW/2,VH-80);
  }
  cx.fillStyle='#5a6b80'; cx.font='11px "Apple SD Gothic Neo",sans-serif';
  cx.fillText(endPage>=3?'탭 — 처음으로':'탭 — 계속',VW/2,VH-30);
}
function endingTap(){
  if(endPage>=3){ document.getElementById('over').classList.add('hidden'); reset(); state='assign'; return; }
  endPage++; endAnimT=0;
}
function realMorning(){
  ...
  else if(journals>=12&&keyLevel>=4&&!deepOpen){ deepOpen=true; state='deepest'; }
  else state='morning';
}
...
function render(){
  ...
  if(state==='deepest'){ drawDeepest(); return; }
  ...
  for(const a of allies){if(a.work&&a.work!=='combat')continue;const img=SPR[a.key];if(!(img.complete&&img.naturalWidth))continue;
    if(a.down>0){drawChar(img,a.x,a.y,52,{gray:true,alpha:.55});cx.fillStyle='#FF5A5A';cx.font='900 10px sans-serif';cx.textAlign='center';cx.fillText('중상 '+Math.ceil(a.down),a.x,a.y-32);continue;}
    const hop=a.moving?-Math.abs(Math.sin(a.phase))*4:0; drawChar(img,a.x,a.y,56,{hop});
...
function loop(ts){if(lastTs===undefined)lastTs=ts;let dt=(ts-lastTs)/1000;lastTs=ts;if(dt>0.05)dt=0.05;
  if(freeze>0){freeze-=dt;} else if(state==='play')update(dt); else if(state==='indoor')updateIndoor(dt); else if(state==='dawn'){dawnT+=dt;if(dawnT>=3.2)realMorning();} else if(state==='ending'||state==='deepest'){endAnimT+=dt;}
  if(state==='play'||state==='levelup'||state==='pause'||state==='dawn'||state==='assign'||state==='morning'||state==='expedition'||state==='memorial'||state==='skill'||state==='indoor'||state==='journal'||state==='deepest'||state==='ending')render();
